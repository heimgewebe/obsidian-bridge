...
Hinweis: Ein basales, deterministisches Topic-Scoping wurde über den `required_tags` Filter etabliert und wird in Themenspezifischen Hubs (wie `topic--observatorium.canvas`) produktiv für präzise inhaltliche Abgrenzungen genutzt. Eine tiefere, allgemeine Topic- oder Investigations-Semantik darüber hinaus ist jedoch noch nicht vollständig ausgebaut und bleibt konzeptionell.
