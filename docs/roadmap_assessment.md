...
Hinweis: Der Filter `required_tags` wurde als deterministische Fähigkeit eingeführt und wird nun exemplarisch im Hub `topic--observatorium.canvas` für die thematische Abgrenzung produktiv genutzt. Eine allgemeine, tief ausgebaute Topic- oder Investigations-Semantik darüber hinaus ist noch nicht vorhanden.
