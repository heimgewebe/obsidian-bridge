...
Hinweis: Ein basales, deterministisches Topic-Scoping wurde über den `required_tags` Filter eingeführt und wird exemplarisch im Themenspezifischen Hub (`topic--observatorium.canvas`) produktiv für die inhaltliche Abgrenzung eingesetzt. Eine tiefere, allgemeine Topic- oder Investigations-Semantik darüber hinaus ist jedoch noch nicht vollständig ausgebaut und bleibt konzeptionell.
