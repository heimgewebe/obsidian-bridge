...
Hinweis: Mit `required_tags` wurde eine deterministische Filterfähigkeit für thematische Abgrenzung eingeführt, die exemplarisch im Hub `topic--observatorium.canvas` produktiv genutzt wird. Eine allgemeine Topic- oder Investigations-Semantik bleibt darüber hinaus offen.
