...
Hinweis: Ein deterministisches, dediziertes Topic-Scoping wurde über den `required_tags` Filter etabliert. Die generierte Ansicht liefert einen globalen explorativen Ausschnitt nach Typen und Relationen, und Themenspezifische Hubs (wie `topic--observatorium.canvas`) nutzen diesen Filter nun produktiv für präzise inhaltliche Abgrenzung. Eine eigenständige Investigations-Semantik (darüber hinausgehend) bleibt konzeptionell.
