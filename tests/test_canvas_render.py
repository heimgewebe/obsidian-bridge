...
class TestCanvasRender(unittest.TestCase):
    ...
    def test_render_canvas_edge_id_consistency(self):
        # This test ensures that the refactored _generate_canvas_edge_id is correctly used
        # and remains behavior-neutral compared to the previous inline logic.
        ...
        graph_data = {
            ...
            "edges": [
                {"id": "test:edge-1", "from": "n1", "to": "n2", "relation": "references"}
            ]
        }
        with open(self.graph_file.name, 'w') as f:
            json.dump(graph_data, f)
        ...
        spec_data = {
            "id": "test-consistency",
            "type": "chronik",
            "output": "canvases/consistency.canvas",
            "source": {"artifact_types": ["event"]}
        }
        with open(self.spec_file.name, 'w') as f:
            yaml.dump(spec_data, f)
        ...
        output_path = os.path.join(self.temp_dir.name, "canvases/consistency.canvas")
        with open(output_path, 'r') as f:
            canvas = json.load(f)
        ...
        # Verify the ID matches what the utility function would generate
        base_edge_id = _get_edge_id(graph_data["edges"][0])
        expected_id = _generate_canvas_edge_id(base_edge_id)

        self.assertEqual(edge["id"], expected_id)
        # Specifically verify the expected format/normalization
        self.assertTrue(edge["id"].startswith("test_edge-1_"))
