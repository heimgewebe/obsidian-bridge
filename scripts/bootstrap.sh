...
echo "Bootstrapping Obsidian Observatorium repo..."
...
if [ ! -f "$REPO_ROOT/config/obsidian.env.example" ]; then
  cat << 'EOF' > "$REPO_ROOT/config/obsidian.env.example"
# Base settings for obsidian scripts
VAULT_PATH="$HOME/Documents/Obsidian"
OBSIDIAN_LOG_FILE="/tmp/obsidian.log"
EOF
fi

if [ ! -f "$REPO_ROOT/config/profiles/home.env" ]; then
  cp "$REPO_ROOT/config/obsidian.env.example" "$REPO_ROOT/config/profiles/home.env"
  echo "Created home profile: config/profiles/home.env"
fi

if [ ! -f "$REPO_ROOT/config/profiles/work.env" ]; then
  cp "$REPO_ROOT/config/obsidian.env.example" "$REPO_ROOT/config/profiles/work.env"
